<script>
    ...
    import {USER, LOGO} from "$lib/store.js";
...
<Page class="" >
    <div class="p-5 bg-blue-900 dark:bg-blue-900 h-screen" transition:fly="{{ x: 200, duration: 200 }}">
        ...
        <div class="mt-10">
            ...
            <h1 class="text-3xl text-white font-bold text-center mt-5 mb-8">Sign up</h1>
        </div>


        <form on:submit={signUpWithEmail}>
        ...
        <button class="flex items-center justify-around gap-x-0 w-full text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none " on:click={signUpWithGoogle}>
            ...
            {:else}
                ...
                <span class="m-auto">Sign up with Google</span>
            {/if}
        </button>
        ...
        <div class="absolute inset-x-0 bottom-3 w-full p-5">
            ...
            <p class="text-white text-lg text-center font-light">Have an account? <a href="/login" class="underline font-bold">Sign in</a> </p>
